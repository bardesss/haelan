...
| **M4b** `sql_query` | One read only `SELECT` over a throwaway per person projection database built fresh for the call: seven tables, no `person_id` column, no credential table, samples excluded, a five second deadline, a 500 row cap and a concurrency of one | In review |
