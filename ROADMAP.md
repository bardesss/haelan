...
Where haelan has got to, and why it got there in that order. Lifted out of the README when the
project went public: thirty rows of milestone letters are the project's own bookkeeping rather
than something a reader needs, but the reasoning under the table is recorded nowhere else - specs
and plans live under `docs/`, which is not tracked.
