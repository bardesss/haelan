...
**M10, M11 and M12 run in that order because each one reads what the one before it writes.**
M11 compares nights after a logged event against nights without one, and until M10c makes logging
one tap there are too few tagged nights for it to say anything honest. M12's first screen is the
glance, and M10a is the payload it reads; building the native screen first would mean building
baselines, sleep assembly and freshness a second time in Kotlin, which is exactly what M12's rule
forbids. The native glance was briefly its own unit of M10 and moved into M12 for that reason: a
screen in the app it is about to replace would be written twice.
