...
| **M6** What only the archive can answer | Source staleness, so a source that quietly stopped reporting says so instead of thinning a chart; all-time records and a milestones timeline; and an Eddington style number, which needs every day on disk to compute at all. Cut into four on 2026-09-15 and shipped as three: M6-0 the eligibility probe, M6a staleness, and M6c the all-time page, which absorbed M6b when the probe falsified the reason for building the page before its contents | Done |
...
| **M9** The glance | The dashboard stops being a smaller copy of the pages beside it and answers three questions instead: how did I sleep, how recovered am I, how is today going. One headline figure each, seven days of context, and a link into the night, the day or the workout behind it; no control row, for the reason `/records` has none. **The payload is the contract, not the page**: one server read returns every figure with its baseline, its *as of* time and its source's staleness, so the web page and the native app in M12 render the same answers and neither recomputes a baseline. Cut into three: M9a the glance payload, M9b the page on top of it, M9c quick logging - preset event chips and a note for today or last night, one tap each, because the events a person logs are what M10 reads | In progress |
| **M9a** The glance payload | One person bound read carrying last night, today's recovery index, resting heart rate and HRV against their baselines, today's steps and active minutes so far, and a seven day strip per figure; every figure names the time it is current to and whether its source has gone stale. Computed on read, with no stored rows, under the isolation suite every read surface carries. `GET /api/v1/p/:personId/glance`, assembled in core behind `PersonQuery.glance` so an MCP tool can read it later. Each figure carries two currencies, because a once-a-day reading has a day but no instant: `asOfDate`, the day it belongs to, and `asOfMs`, the last reading behind it, never a write time. Last night is the night that ended most recently in the last 36 hours, chosen by when it ended rather than the date it is filed under. Recovery is always present and falls back to yesterday's figures until today's sync, saying which day it shows. A source is stale on a figure when it fed that metric in the 60 days before and has since gone quiet by M6a's own cadence rule, which a seven day look-back could never have seen past its 14 day floor. Values are rounded at the boundary like `/series`, and nothing time-varying sits in the hashed body, so a poll that finds nothing new is answered 304 | In review |
| **M9b** The glance page | The dashboard rebuilt on M9a. Last night leads beside a recovery score and its two dials, with today beside the week below them, and every figure that has a usual range is read against it. The period insight cards leave it, since each already has a home on a page with a range, and the three views only the dashboard had move to one: the heart rate range and a day's trace to Recovery, flagged days to Notes | In review |
| **M9c** Quick logging | Preset event chips a person can edit, and a quick note, logged against today or last night over the annotation routes M3c already ships | Not started |
| **M10** What moves my sleep | Nights following a logged event set against nights that do not, stated as counts and differences in the register `/records` uses rather than as a correlation coefficient. It needs tagged nights to exist before it can say anything, which is why it follows M9c. It closes with the README rewritten shorter and led by what the app does | Not started |
| **M11** Gym set logging | Strength sets (exercise, kg, reps) as a **local overlay** on the Health exercise session they overlap in time, never written back to Google and never joined on `exerciseType`, since a watch logs gym as `WORKOUT` as often as `WEIGHTLIFTING`. The point is the MCP server: Google keeps the physiology of a gym session and not a single lift, so an agent can say "a 40 minute workout" and nothing about bench against row. That makes it the first feature to need **write** tools on a surface that is read-only by design, which is a token policy question before it is a schema one. Specified in [#252](https://github.com/bardesss/haelan/issues/252), which carries the design and the alternatives already rejected. Listed as M9 while it was only a wishlist entry; when it was scheduled after M10 it became M11, and M9 went to the glance | Not started |
| **M12** The native phone app | The Android app becomes the phone UI: Kotlin, Jetpack Compose and Material 3, with the companion sync moved behind an ordinary Settings screen and the sync engine kept as it is. The web stays the desktop app and the fallback for every other device. It opens with a bottom bar, a native glance screen reading M9a, Sleep and Settings; a page not yet native opens in a web view inside the native frame as a migration step only, then one page per pull request. The rule it runs on: **logic that would exist in both clients moves to the server.** Material 3 components take Material You's dynamic colour where the device offers it, from Android 12, and fall back to a scheme built from the palette `packages/tokens` generates on older devices. **Dynamic colour reaches the chrome only**: sleep stages, heart rate zones, the better and worse colours on a change badge and every warning keep their token colours on every device, because a wallpaper must never make deep sleep and REM indistinguishable or turn a worse week green, and a chart should read the same in the app as on the web. The tokens' Android target emits only colours today, so spacing, radius and weight join it here | Not started |
...
**M9, M10 and M12 run in that order because each one reads what the one before it writes.**
M10 compares nights after a logged event against nights without one, and until M9c makes logging
one tap there are too few tagged nights for it to say anything honest. M12's first screen is the
glance, and M9a is the payload it reads; building the native screen first would mean building
baselines, sleep assembly and freshness a second time in Kotlin, which is exactly what M12's rule
forbids. The native glance was briefly its own unit of M9 and moved into M12 for that reason: a
screen in the app it is about to replace would be written twice. M11, gym set logging, depends on
neither and sits between them by choice rather than by dependency.
...
**A milestone is not done until the demo and the screenshots show it.** From M9 on, the pull
request that finishes a milestone also re-captures the public demo with `pnpm demo:capture` and
retakes the screenshots in `assets/screenshots` off the same seeded data. That applies whenever
the milestone changed anything a page renders; a milestone that changed none says so in its pull
request instead. The demo replays captured responses, so a page reading a route the capture
has never seen does not look slightly old. It answers `not_found`, and it does so on the page a
