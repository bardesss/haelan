...
| **M5** Packaging | Cut into six units: M5a source naming, M5b people, M5c packaging, M5d backup and the upgrade path, M5e documentation and the envelope migration, M5f wizard polish and the connect surface M5b left out, with the catalogue work landing before the v1.0.0 tag so the first published image is not shipped with the catalogue behind the API. Macronutrients are settled rather than left as ingestion work: they live on `nutrition-log`, whose mapping is deferred for a stated reason, and the `food` type that was expected to carry them has no timestamp of any kind | In progress |
