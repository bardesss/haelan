...
describe('what the caller\'s SQL cannot reach', () => {
  ...
  it('cannot load an extension', async () => {
    await expect(run("SELECT load_extension('/tmp/x.so')")).rejects.toThrow(ConfigError)
  })

  it('cannot read a file', async () => {
    await expect(run("SELECT readfile('/etc/hostname')")).rejects.toThrow(ConfigError)
  })
...
describe('the deadline', () => {
  it('gives up on a CPU-bound query and leaves the main thread serving', async () => {
    ...
    const query = run('WITH RECURSIVE c(x) AS (SELECT 1 UNION ALL SELECT x+1 FROM c WHERE x < 2000000000) SELECT count(*) FROM c')
    let ticks = 0
