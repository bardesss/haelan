...
export function deepLink(
  ...
  return withQuery(path, {
    range: controls.tab,
    on: controls.anchor,
    // Omitted rather than written out, so a link says merged by not saying anything. Pinning the
    // default would make every link carry a parameter that changes nothing.
    source: controls.source === 'merged' ? null : controls.source,
  })
}
