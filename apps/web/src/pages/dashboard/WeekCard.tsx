...
import type { Glance, GlanceWeekFigure } from '../../data/useGlance.js'
...
export function WeekCard({ glance, span }: { glance: Glance, span: 4 | 8 | 12 }) {
  ...
  const rows: { key: 'steps' | 'active' | 'asleep', figure: GlanceWeekFigure, tone: 'steps' | 'active' | 'sleep', values: (number | null)[], value: string, basis: string }[] = []
  ...
  if (glance.week.asleep !== null && glance.sleep !== null) {
    rows.push({
      ...
      basis: t('glance.week.perNight'),
    })
  }
  return (
    <DashCard span={span} title={t('glance.week.title')} subtitle={t('glance.week.subtitle')} staleFigures={[]}>
      {rows.map((row) => (
        <div className="dash-week-row" key={row.key}>
          ...
          <WeekBars values={row.values} tone={row.tone} label={t('glance.week.barsLabel', { what: t(`glance.week.${row.key}`) })} />
        </div>
      ))}
    </DashCard>
  )
