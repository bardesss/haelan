...
describe('MetricCard', () => {
  ...
  it('renders the error state for a failed request, not an empty period', () => {
    mount(<MetricCard metric="steps" span={1} basisPlacement="body" query={{ isError: true, isPending: false, refetch: () => {} }}
  ...
  it('still shows the error when a composite query is both errored and pending', () => {
    mount(<MetricCard metric="steps" span={1} basisPlacement="body" query={{ isError: true, isPending: true, refetch: () => {} }}
      points={[]} basisKey="b" basisWornKey="bw">{() => <span>drawn</span>}</MetricCard>)
    expect(container!.textContent).toContain('errorState.title')
  ...
  it('claims nothing at all while the query is pending', () => {
    mount(<MetricCard metric="steps" span={1} basisPlacement="body" query={{ isError: false, isPending: true, refetch: () => {} }}
      points={[]} basisKey="b" basisWornKey="bw">{() => <span>drawn</span>}</MetricCard>)
  ...
  it('renders a genuine zero as data rather than as emptiness', () => {
    mount(<MetricCard metric="steps" span={1} basisPlacement="body" query={OK} points={[point(0, 0.9)]}
  ...
  it('chooses the plain basis for a metric that carries no wear signal', () => {
    mount(<MetricCard metric="sleep_asleep_minutes" span={1} basisPlacement="body" query={OK} points={[point(420, null)]}
      basisKey="b" basisWornKey="bw">{(basis) => <span>{basis}</span>}</MetricCard>)
  ...
  it('prints the basis exactly once when placement is header', () => {
    mount(<MetricCard metric="steps" span={1} basisPlacement="header" query={OK} points={[point(900, 0.9)]}
