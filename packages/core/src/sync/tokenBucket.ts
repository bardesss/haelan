...
export class TokenBucket {
  ...
  readonly #config: { capacity: number, refillPerMinute: number }
  readonly #deps: TokenBucketDeps

  constructor(
    config: { capacity: number, refillPerMinute: number },
    deps: TokenBucketDeps = { now: Date.now, sleep: (ms) => new Promise((r) => setTimeout(r, ms)) },
  ) {
    this.#config = config
    this.#deps = deps
