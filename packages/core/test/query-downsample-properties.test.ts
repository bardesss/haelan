...
describe('thinning properties', () => {
  ...
  it('never moves the series minimum or maximum outside the original range', () => {
    fc.assert(fc.property(seriesArb, fc.integer({ min: 2, max: 50 }), (points, target) => {
      const out = thin(points, target, { method: 'lttb', x: (p) => p.x, y: (p) => p.y })
  ...
  it('never returns more points than asked for', () => {
    fc.assert(fc.property(seriesArb, fc.integer({ min: 2, max: 50 }), (points, target) => {
      expect(thin(points, target, { method: 'lttb', x: (p) => p.x, y: (p) => p.y }).points.length)
