...
export default defineConfig({
  ...
  test: {
    ...
    testTimeout: 20_000,
    // The same budget for hooks, which vitest otherwise leaves at its own 5s default while the
    // number above is deliberate. Every word of the reasoning above applies to a beforeEach
    // exactly as it applies to a test body: the hook runs under the same uncapped parallelism,
    // gets the same share of CPU, and has the same 2x margin to lose. Leaving the two to drift
    // meant a hook had a tighter budget than the test it was setting up, which is backwards -
    // setup is where this suite does its expensive work, since a test that needs rows has to
    // write them first.
    //
    // session-heart-rate.test.ts is what found it: its beforeEach writes 700 minutes across
    // three aggregates, 2100 rows before each of seven tests, and the file needs about 21s alone.
